...
基于 Lua 的个人 Neovim 配置，使用 [packer.nvim](https://github.com/wbthomason/packer.nvim) 管理插件。
...
3. **启动 Neovim：**
   ...
   首次启动时会自动安装 **packer.nvim**，随后通过 `:PackerSync` 安装所有插件。
...
保存 `plugins-setup.lua` 时也会自动触发同步。
